...
* Roughly how much time you spent on this HW so far: 6
...
bitcoin_changes <- bitcoin %>% group_by(Date) %>% mutate(next_day = Date + 1) 
...
ggplot(data=bitcoin_data, aes(x=Date, y=relative_change)) +geom_point()


gold <- Quandl("BUNDESBANK/BBK01_WT5511") %>%
  tbl_df() 
ggplot(data=gold, aes(x=Date, y=Value)) +geom_point()
gold_changes <- gold %>% group_by(Date) %>% mutate(next_day = Date + 1) 
gold_data <- full_join(gold_changes, gold, by= c("next_day"= "Date")) %>% mutate(relative_change = (Value.y-Value.x)/Value.x) %>% filter(!is.na(relative_change)) 
ggplot(data=gold_data, aes(x=Date, y=relative_change)) +geom_point()
...
```{r, echo=TRUE, message=TRUE, warning=TRUE, fig.width=8, fig.height=4.5}
graveyard <- jukebox %>% mutate(date_time=parse_date_time(date_time,"a b! d! H: M:S Y!"))
graveyard <- graveyard %>% mutate(hour = hour(date_time)) %>% filter(hour>= 0 & hour <= 8) 
graveyard_artist <- graveyard %>% group_by(artist) %>% tally()
graveyard_artist %>% top_n(10)
...
academic_artist <- academic_year %>% group_by(artist) %>% tally()
academic_artist %>% top_n(10)
